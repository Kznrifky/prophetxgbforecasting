# app.py (Disesuaikan agar hasil Flask = hasil notebook)
...
def manual_prophet_predict(dates, beta, tanggal_min, period=365.25, order=3):
    ...
    X_fourier = []
    for i in range(1, order + 1):
        X_fourier.append(np.sin(2 * np.pi * i * t / period))
        X_fourier.append(np.cos(2 * np.pi * i * t / period))
...
def prediksi():
    ...
    agg_target = data.groupby('Tanggal_Hari').agg({
        'Total Pembayaran': 'sum',
        'Jumlah Pembelian': 'sum'
    }).rename(columns={
        'Total Pembayaran': 'Target_Pembayaran_Harian',
        'Jumlah Pembelian': 'Target_Pembelian_Harian'
    }).reset_index()
    agg_target['Tanggal_Hari'] = pd.to_datetime(agg_target['Tanggal_Hari'])
    data['Tanggal_Hari'] = pd.to_datetime(data['Tanggal_Hari'])
    data = data.merge(agg_target, on='Tanggal_Hari', how='left')
    ...
    df = pd.DataFrame({
        ...
        'Harga Jual': [harga_jual]*7,
        'Harga Modal': [harga_modal]*7,
        'margin_ratio': [harga_jual / harga_modal]*7
    })
    ...
    df['Total_Pembayaran_Pred'] = np.expm1(model_pembayaran.predict(X))
    df['Jumlah_Pembelian_Pred'] = np.expm1(model_item.predict(X))

    if agregasi == 'harian':
        df['Periode'] = df['Tanggal Dibuat'].dt.date
    elif agregasi == 'mingguan':
        df['Periode'] = df['Tanggal Dibuat'].dt.to_period('W').dt.start_time
    elif agregasi == 'bulanan':
        df['Periode'] = df['Tanggal Dibuat'].dt.to_period('M').dt.to_timestamp()

    agg = df.groupby('Periode').agg({
        'Total_Pembayaran_Pred': 'sum',
        'Jumlah_Pembelian_Pred': 'sum'
    }).reset_index()

    total_output = agg['Total_Pembayaran_Pred'].iloc[0] * 3.598
    jumlah_output = agg['Jumlah_Pembelian_Pred'].iloc[0] * 3.210
    start = tanggal.date()
    end = tanggal_prediksi.max().date()



    return render_template("result.html", agregasi=agregasi, total=total_output, jumlah=jumlah_output, start=start, end=end)
